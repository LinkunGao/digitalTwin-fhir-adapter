...
class Test:
    ...
    async def test_search_digital_twin(self):
        ...
        workflow_tool = await client.resources("ActivityDefinition").search(identifier="sparc-workflow-tool-clinical_report-777").first()
        ...
        print("inputs: ", inputs)
        for i in inputs:
            composition = await client.resources("Composition").search(title="primary measurements", entry=i.get("valueReference")).first()
